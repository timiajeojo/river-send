...
function Toggle({ enabled, onToggle }: { enabled: boolean; onToggle: () => void }) {
  return (
    <button type="button" onClick={onToggle} className={`relative w-11 h-6 rounded-full border-none cursor-pointer transition-colors duration-200 shrink-0 ${enabled ? "bg-[#AAFF00]" : "bg-gray-200"}`}>
  ...
  return (
    <div className="flex items-center justify-between gap-3 px-4 py-4 sm:px-5">
      <div className="flex-1 min-w-0 pr-2">
        ...
        <p className="text-[12px] text-gray-400 mt-0.5 truncate">{sub}</p>
      </div>
