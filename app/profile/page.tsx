...
const menuItems = [
  {
    ...
    icon: <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>,
  },
  {
    label: "Security",
  ...
  },
  {
    ...
    badge: true,
    icon: <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg>,
  },
  {
    label: "Support",
    route: "/profile/support",
    icon: <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>,
  },
];
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto pb-24 lg:pb-10">
          <div className="px-4 pt-5 sm:px-5 sm:pt-6 lg:px-8 lg:pt-8">
            ...
            {/* Desktop: two-col | Mobile+Tablet: single col */}
            <div className="flex flex-col gap-4 lg:grid lg:grid-cols-3 xl:grid-cols-4 lg:gap-6 lg:items-start">

              {/* Profile card — full width mobile, 1 col desktop */}
              <div className="lg:col-span-1">
                <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                  <button
                    type="button"
                    onClick={() => router.push("/profile/personal-info")}
                    className="w-full flex items-center gap-3 px-5 py-4 sm:py-5 text-left border-none cursor-pointer hover:bg-gray-50 transition-colors"
                  >
                    <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full overflow-hidden shrink-0 ring-2 ring-[#AAFF00]/30">
                      <img src="https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?w=100&q=80&auto=format&fit=crop&crop=face" alt="avatar" className="w-full h-full object-cover" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[15px] sm:text-[16px] font-bold text-gray-900">Hello Tega</p>
                  ...
                  {/* Stats — desktop only inside card */}
                  <div className="hidden lg:grid grid-cols-2 gap-px bg-gray-100 border-t border-gray-100">
                    {[{ val: "$8,630", label: "Balance" }, { val: "12", label: "Transactions" }].map((s) => (
              ...
              {/* Menu items — 2 or 3 cols on desktop */}
                  ...
                  {menuItems.map((item, i) => (
                    <div key={item.label}>
                      <button
                        type="button"
                        onClick={() => router.push(item.route)}
                        className="w-full flex items-center gap-4 px-5 py-4 sm:py-4.5 text-left border-none cursor-pointer hover:bg-gray-50 transition-colors"
                      >
                        <span className="text-gray-500 shrink-0">{item.icon}</span>
                        <span className="flex-1 text-[14px] sm:text-[15px] font-medium text-gray-900">{item.label}</span>
                        {item.badge && <span className="w-2.5 h-2.5 rounded-full bg-green-500 block shrink-0" />}
                        <svg className="w-4 h-4 text-gray-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                          <polyline points="9 18 15 12 9 6"/>
                        </svg>
                      </button>
