...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-svh w-full bg-white flex flex-col lg:flex-row">

      {/* ── LEFT PANEL — desktop only ── */}
      <div className="hidden lg:flex flex-col justify-end flex-1 relative bg-[#0d0d0d] overflow-hidden p-14 min-h-screen">
        {/* Radial glow */}
        <div
          className="absolute inset-0 pointer-events-none"
          style={{
            background:
              "radial-gradient(ellipse 80% 60% at 50% 80%, rgba(170,255,0,0.12) 0%, transparent 70%), radial-gradient(ellipse 40% 40% at 20% 20%, rgba(170,255,0,0.06) 0%, transparent 60%)",
          }}
        />
      ...
      {/* ── RIGHT PANEL — form ── */}
      <div className="flex flex-col w-full lg:w-[480px] xl:w-[520px] lg:shrink-0 min-h-svh bg-white overflow-y-auto">
        <div className="flex flex-col w-full max-w-[480px] mx-auto px-6 pt-8 pb-12 sm:px-10 lg:px-12 lg:pt-12">
          ...
          {/* Greeting */}
          <h2 className="text-[26px] sm:text-[28px] lg:text-[30px] font-bold tracking-tight text-gray-900 mb-1.5">
          ...
          {/* Tab switcher */}
          <div className="flex w-full border-b-2 border-gray-100 mb-8">
            {(["signin", "signup"] as AuthMode[]).map((m) => (
              <button
                ...
                className={`flex-1 pb-3 text-[15px] font-semibold cursor-pointer border-none bg-transparent transition-colors duration-200 relative ${
                  mode === m ? "text-gray-900" : "text-gray-400 hover:text-gray-600"
                }`}
              >
