...
function TransactionLogo({ tx }: { tx: Transaction }) {
  if (tx.logo === "spotify") return (
    <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-[#1DB954] flex items-center justify-center shrink-0">
  ...
  if (tx.logo === "amazon") return (
    <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-[#232F3E] flex items-center justify-center shrink-0">
      <span className="text-[#FF9900] font-black text-[16px]">a</span>
    </div>
  );
  if (tx.logo === "netflix") return (
    <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-[#E50914] flex items-center justify-center shrink-0">
      <span className="text-white font-black text-[15px]">N</span>
    </div>
  );
  return (
    <div className={`w-10 h-10 sm:w-11 sm:h-11 rounded-full flex items-center justify-center shrink-0 ${tx.bg}`}>
      {tx.positive ? (
  ...
  return (
    <div className="lg:mb-0">
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <p className="text-[15px] sm:text-[16px] font-bold text-gray-900">Recent Transactions</p>
        <button type="button" className="text-[13px] text-gray-400 bg-transparent border-none cursor-pointer hover:text-gray-600 underline underline-offset-2">
          See all
        </button>
      </div>
      <div className="flex flex-col bg-white rounded-2xl overflow-hidden shadow-sm">
        {recent.map((tx, i) => (
          <div
            key={tx.id}
            className={`flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-3.5 sm:py-4 ${i < recent.length - 1 ? "border-b border-gray-50" : ""}`}
          >
            <TransactionLogo tx={tx} />
            <div className="flex-1 min-w-0">
              <p className="text-[14px] sm:text-[15px] font-semibold text-gray-900 truncate">{tx.name}</p>
              <p className="text-[11px] sm:text-[12px] text-gray-400 mt-0.5">{tx.date} · {tx.time}</p>
            </div>
            <span className={`text-[14px] sm:text-[15px] font-bold shrink-0 ${tx.positive ? "text-[#22c55e]" : "text-gray-900"}`}>
              {tx.amount}
            </span>
          </div>
        ))}
